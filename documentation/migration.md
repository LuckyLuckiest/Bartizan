...
2. Drop `Bartizan-0.1.0.jar` into `/plugins`, beside `Keystone-1.9.0.jar` and `Gangland_Warfare-0.9.0.jar`.
3. Restart the server (Bartizan, like every Keystone-based plugin here, wires its bean graph at `onEnable` — no
   hot-reload of the jar itself).

Bartizan soft-depends on `ViaVersion`, `PlaceholderAPI` and `NBTAPI`; none is required.
...
jetpack:
   ...
   Jetpack:
      ...
      Sound:
         Thrust:
            Default_Sound: { Sound: ENTITY_BLAZE_SHOOT, Volume: 0.6, Pitch: 1.8 }
         Glide:
            Default_Sound: { Sound: ITEM_ELYTRA_FLYING, Volume: 0.4, Pitch: 1.0 }
```
...
| Key | Meaning |
|---|---|
| `Auto_Save.Time` (minutes, default `10`) | General auto-save cadence — **not** the weapon table cleanup schedule (see below). |
