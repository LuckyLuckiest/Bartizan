package org.luckyraven.bartizan.testsupport;
 ...
 * {@code XAttribute.ARMOR.get()}, and (the reason {@code bartizan-plugin} needs its own copy) any code path that
 * merely needs {@code Bukkit.getItemFactory()} to be non-null to avoid an NPE — {@code Weapon#buildItem}'s
 * {@code ItemBuilder} calls route display-name/lore/durability writes through
 ...
 *
 * <p><b>Upstream candidate:</b> this belongs in {@code keystone-testkit} beside {@code BukkitStatics}.
 *
 * <p><b>Duplicated from {@code bartizan-api}'s copy</b> (bartizan.md §3 N-gap, same rationale as B19's
 * {@code WeaponFixtures} duplication: {@code bartizan-plugin} test classes cannot depend on {@code bartizan-api}'s
 * test-jar). Package changed only.
 */
public final class BukkitRegistryFixture {
	 ...
	 * {@code ClassCastException} at the call site, not here). Fixed here versus {@code bartizan-api}'s original
	 * copy of this fixture, which has not yet hit a registry-proxied {@code short}-returning call (N-gap finding,
	 * recorded — not touched there since {@code bartizan-api} main+test sources are otherwise frozen and no test
	 * there currently needs it).
	 */
