 ...
 *
 * <p>Used by both {@code BiologicalAction} and the throwable smoke/stun subtypes so the token grammar stays
 * consistent across the weapon system. (Deviation, recorded in bartizan.md §7 task B12: the original javadoc used
 * {@code @link BiologicalAction}, but that class is a group-H file that does not exist yet when this file compiles
 * — {@code @code} avoids the unresolved-import compile error the checklist's gate-GA review already fixed twice
 * for the same class of stale cross-group javadoc link in B5/B6.)
 *
