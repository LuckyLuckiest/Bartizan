...
/**
 * The standalone-plugin twin of Gangland's {@code GanglandDatabase} (bartizan.md §1.9): same legacy-pool +
 * {@link DatabaseBackend} connection lifecycle, schema name {@code "bartizan"} so the SQLite file lands at
 * {@code plugins/Bartizan/database/bartizan.db}. {@link #insertInitialData()} is an empty override — Bartizan has
 * no rank seeding to do, unlike Gangland's copy.
 */
public class BartizanDatabase extends DatabaseHandler {
	...
	public BartizanDatabase(JavaPlugin plugin, String schema, DatabaseSettingsProvider settings) {
	...
	@Nullable
	public static BartizanDatabase findInstance(DatabaseManager manager) {
		return manager.getDatabases()
				.stream()
				.filter(handler -> handler instanceof BartizanDatabase)
				.map(BartizanDatabase.class::cast)
				.findFirst()
				.orElse(null);
	}
	...
	public void insertInitialData() throws SQLException {
		// Bartizan has no seed data — unlike Gangland's copy, which inserts the default rank hierarchy here.
	}
